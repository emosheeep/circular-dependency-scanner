 ...
 */
async function getImportNodes(content: string) {
 ...
 */
async function getTypeExcludedImportNodes(content: string) {
  const sgNode = await parseAsync(Lang.Tsx, content);
  return sgNode.root().findAll({
    utils: {
      'is-not-follows-type': {
...
function getScriptContentFromVue(filename: string) {
  const { descriptor: result } = vueParse(fs.readFileSync(filename, 'utf-8'));
  const { script, scriptSetup } = result;
  const scriptNode = script || scriptSetup;
  return scriptNode?.content;
}

export async function getImportSpecifiers(
  filePath: string,
  excludeTypes = false,
): Promise<string[]> {
  ...
  const nodes = excludeTypes
    ? await getTypeExcludedImportNodes(fileContent)
    : await getImportNodes(fileContent);
